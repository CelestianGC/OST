...
import org.ost.main.MyClasses.WeaponProfList;
 ...
 * @author  __USER__
 */
public class Option_Set_WeaponProfs extends javax.swing.JDialog {
	private MainClass ost;
	private java.awt.Frame parent;
	private ArrayList<WeaponProfClass> eCurrent;
	private WeaponProfList eList;

	/** Creates new form Option_Set_Race */
	public Option_Set_WeaponProfs(java.awt.Frame parent, boolean modal,
			MainClass ost, ArrayList<WeaponProfClass> setCurrent) {
		super(parent, modal);
		this.parent = parent;
		this.ost = ost;
		this.eCurrent = setCurrent;
		this.eList = ost.WeaponProfList;
		initComponents();
	...
	private void updateList(JList list, WeaponProfList aList) {
	...
	private void updateAllowed(JList list, ArrayList<WeaponProfClass> aList) {
		DefaultListModel model = (DefaultListModel) list.getModel();
		Object oSelected = list.getSelectedValue();
		model.removeAllElements();
		for (Object o : aList)
		...
		if (!oSelect.isEmpty()) {
			for (Object oNew : oSelect)
				...
				if (oNew != null && !eCurrent.contains(oNew)) {
					eCurrent.add((WeaponProfClass) oNew);
				}
			updateAllowed(extraAllowedList, eCurrent);
			updateList(extraListList, eList);
		}
