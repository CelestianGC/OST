/*
 * Option_Set_ExtraAbilities.java
 *
 ...
 * @author  __USER__
 */
public class Option_Set_ExtraAbilities extends javax.swing.JDialog {
	private MainClass ost;
	private int typeFilter;
	private java.awt.Frame parent;
	private ArrayList<ExtraAbilitiesClass> eCurrent;
	private ExtraAbilitiesList eAbilities;

	/** Creates new form Option_Set_ExtraAbilities */
	public Option_Set_ExtraAbilities(java.awt.Frame parent, boolean modal,
			MainClass ost, ArrayList<ExtraAbilitiesClass> setCurrent) {
		super(parent, modal);
		this.parent = parent;
		this.ost = ost;
		this.typeFilter = -1;
		this.eCurrent = setCurrent;
		this.eAbilities = ost.extraAbilitiesList;
		initComponents();
	...
	private void updateList(JList list, ExtraAbilitiesList aList) {
		DefaultListModel model = (DefaultListModel) list.getModel();
		Object oSelected = list.getSelectedValue();
		model.removeAllElements();

		for (Object o : aList.getContent()) {
			ExtraAbilitiesClass oE = (ExtraAbilitiesClass) o;
			if ((typeFilter > -1 && oE.getType() == typeFilter)
					|| typeFilter == -1)
	...
	private void updateAllowed(JList list, ArrayList<ExtraAbilitiesClass> aList) {
		DefaultListModel model = (DefaultListModel) list.getModel();
		Object oSelected = list.getSelectedValue();
		model.removeAllElements();
		for (Object o : aList)
	...
	private void initComponents() {
		...
		extraModifyButton.setText("modify");
		extraModifyButton
		...
		filterButton.setFont(new java.awt.Font("Segoe UI", 0, 10));
		filterButton.setText("filter");
		filterButton.setToolTipText("Filter list.");
		filterButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				filterButtonActionPerformed(evt);
			}
		});
		extraAllowedButtonPanel1.add(filterButton);
	...
	private void filterButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		Option_AskFor_Filter dDialog = new Option_AskFor_Filter(parent, true,
				ost, "Filter", EXTRA_ABILITY_NAMES);
		dDialog.setVisible(true);

		// all done, grab filter number
		typeFilter = dDialog.currentType;
		

		//update the list of abilities with the new filters
		updateList(extraListList, eAbilities);
		...
		if (!oSelect.isEmpty()) {
			for (Object oNew : oSelect)
				...
				if (oNew != null && !eCurrent.contains(oNew)) {
					eCurrent.add((ExtraAbilitiesClass) oNew);
				}
			updateAllowed(extraAllowedList, eCurrent);
			updateList(extraListList, eAbilities);
		}
